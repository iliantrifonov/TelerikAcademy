...
//                    console.log(data);
    ...
    all: function (req, res, next) {
        ...
        if (options.search){
            findObj.url = new RegExp(options.search, "i");
        }
        ...
        File.find(findObj)
            ...
            .page(mongooseOptions, function(err, data) {
                ...
                if(!data){
                    res.render(controllerName + '/list', { files: data, options: options });
                    return;
                }

                for (var i = 0, len = data.length; i < len; i++){
                    var cipher = data[i].url;
                    var path = encryption.decrypt(cipher, encryptionPass);

                    var parts = path.split('/');
                    data[i].fileName = parts[parts.length - 1];
                }
